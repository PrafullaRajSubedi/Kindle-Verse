...
import mode from "../assets/modern.jpg";
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-4">
        {books.map((book) => (
          <div key={book.id} className="group">
            <div className="aspect-w-3 aspect-h-4 max-w-[120px] w-full overflow-hidden rounded-lg bg-gray-100 mb-3">
              <img
                src={mode}
                alt={book.title}
                className="object-cover object-center group-hover:opacity-75 max-h-40"
              />
            </div>
...
export default function BookstoreCartPage() {
  ...
  // Apply code discount if valid
  const codeDiscountRate = appliedDiscountCode
    ? VALID_DISCOUNT_CODES[appliedDiscountCode].rate
    : 0;
  ...
  // Handler to add recommended book to cart
    ...
    if (existingItemIndex >= 0) {
      ...
      setCartItems([
        ...cartItems,
        {
          ...book,
          quantity: 1,
          stockCount: 10, // Default stock count for recommended books
  ...
  const handleApplyDiscountCode = (code) => {
    ...
    if (VALID_DISCOUNT_CODES[upperCaseCode]) {
      setAppliedDiscountCode(upperCaseCode);
      return {
        success: true,
        message: VALID_DISCOUNT_CODES[upperCaseCode].message,
      };
    } else {
      return {
        success: false,
        message: "Invalid discount code. Please try another.",
      };
    }
  };
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      setOrderDetails({
        ...
        claimCode: created.orderNumber, // or whatever your API returns
