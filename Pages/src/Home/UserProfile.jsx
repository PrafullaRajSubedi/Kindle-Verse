...
export default function UserProfile() {
  ...
  const [edit, setEdit] = useState({ firstName: "", lastName: "", password: "" });
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        ...
        setUser(res.data);
        setEdit({ firstName: res.data.firstName, lastName: res.data.lastName, password: "" });
      } catch (err) {
    ...
    <div className="bg-gray-50 min-h-screen w-full">
      <main className="w-full mx-auto px-4 py-8 flex flex-col md:flex-row">
        ...
        <section className="flex-1 p-6 bg-white shadow rounded-lg">
            ...
            <>
              ...
              {showSaved && (
                <div className="mb-4 flex items-center text-green-700 bg-green-100 p-3 rounded">
                  <CheckCircle className="h-5 w-5 mr-2" /> Profile updated successfully!
                </div>
              )}
              <div className="space-y-4">
                ...
                <div className="flex gap-4 pt-2">
                  ...
                  <Button variant="secondary" onClick={() => setEdit({ ...edit, password: "" })}>
                    Cancel
                  </Button>
                </div>
              </div>
          ...
          {activeTab === "discounts" && (
            <div className="text-gray-600">Loyalty discounts will be available here.</div>
          )}
        </section>
      </main>
    </div>
