...
export default function Home() {
  ...
  const [booksPerPage] = useState(4); // Show 4 books per page (1 row of 4 books)
  ...
  useEffect(() => {
    const runSearch = async () => {
      ...
      try {
        const response = await axios.get(
          "http://localhost:5126/api/BookFilter/list",
          {
            params: query,
          }
        );
  ...
  return (
    <div className="flex flex-col min-h-screen bg-neutral-50">
      {/* Hero Section - Only show if not displaying search results */}
      {!isSearchResults && (
      ...
      {/* Genres section - Only show if not displaying search results */}
      {!isSearchResults && (
      ...
      {/* Search Results Header - Only show when displaying search results */}
      {isSearchResults && (
        <section className="py-8 bg-white">
              ...
              Object.keys(activeSearchParams).length > 0 && (
                <div className="bg-neutral-50 rounded-lg p-4 mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {activeSearchParams.author && (
                      <div className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm">
                        Author: {activeSearchParams.author}
                      </div>
                    )}
                    {activeSearchParams.genre && (
                      <div className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm">
                        Genre: {activeSearchParams.genre}
                      </div>
                    )}
                    {activeSearchParams.minPrice && (
                      <div className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm">
                        Min Price: ${activeSearchParams.minPrice}
                      </div>
                    )}
                    {activeSearchParams.maxPrice && (
                      <div className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm">
                        Max Price: ${activeSearchParams.maxPrice}
                      </div>
                    )}
                    {activeSearchParams.sortBy && (
                      <div className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm">
                        Sort: {activeSearchParams.sortBy} (
                        {activeSearchParams.sortOrder || "asc"})
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="mt-4">
            <button
              onClick={() => (window.location.href = "/")} // Clears search params
              className="bg-blue-700 hover:bg-blue-800 text-white px-6 py-2 rounded-lg shadow transition"
            >
              Back to Home
            </button>
          </div>
      ...
      {/* Books Section - Always show */}
      <section className="py-16 bg-neutral-50">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-3xl font-bold">
              {isSearchResults ? "Search Results" : "Featured Books"}
            </h2>
          </div>
                ...
                <div key={book.id} className="group">
                  <div className="bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 hover:shadow-xl">
                    <div className="relative">
                      <img
                        src={book.coverImagePath || harryp}
                        alt={book.title}
